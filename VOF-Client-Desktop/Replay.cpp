...
QString replay::buildReplayPath(int gameId)
{
...
int replay::findNextFreeReplayId()
{
    for (int i = 1; i <= 10; ++i)
    {
        QString path = buildReplayPath(i);
        if (!QFile::exists(path))
            return i;
    }
    return 1; // fallback: älteste überschreiben
}
...
void replay::on_importBtn_clicked()
{
    ...
    QString basePath = QCoreApplication::applicationDirPath() + "/Replays/";
    int targetId = replay::findNextFreeReplayId();
